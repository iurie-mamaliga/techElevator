...
public class Exercises {
	...
	public String twoChar(String str, int index) {
		...
		// ja  jav  java javas 5 2 javagtfd 8 5
	}
	...
	public boolean hasBad(String str) {
		if(str.length() < 5)
			if(str.length() < 3)
				return false;
			else if((str.substring(0, 3).equals("bad") || str.substring(1, str.length()).equals("bad")))
				return true;
			else
				return false;
	...
	public boolean doubleX(String str) {
		...
		for(int i = 0; i < str.length(); i++) {
			if(str.charAt(str.charAt(str.length()-2)) == 'x' && str.charAt(str.length()-1) == 'x')
				return true;
			else if (str.charAt(str.charAt(str.length()-2)) != 'x' || str.charAt(str.length()-1) != 'x')
				return false;
	...
	public int last2(String str) {
		...
		for(int i = 0; i < str.length() -2; i++) {
			if(str.length() == 0 || str.length() == 1 || str.length() == 2 || str.length() == 3)
				return 0;
			else if(str.substring(i, i + 2).equals(temp))
				count = count + 1;
		}
		return count;
	}
	...
	public String altPairs(String str) {
		...
		while(i < str.length()) {
			...
			else if(i != str.length() && i < str.length())
				temp += str.charAt(i + 1);					
			i = i + 4;
		}
		return temp;
