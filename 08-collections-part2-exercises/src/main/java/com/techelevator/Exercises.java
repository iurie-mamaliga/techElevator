...
public class Exercises {
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		if (peterPaul.get("Peter") > 0 && peterPaul.get("Paul") < 1000) {
			...
			int tempPeter = peterPaul.get("Peter");
			int tempPaul = peterPaul.get("Paul");

			if (tempPeter % 2 == 0) {
				tempPeter = tempPeter / 2;
				peterPaul.put("Peter", tempPeter);
				peterPaul.put("Paul", tempPeter + tempPaul);
			} else {
				tempPeter = tempPeter / 2 + 1;
				peterPaul.put("Peter", tempPeter);
				peterPaul.put("Paul", (tempPeter - 1) + tempPaul);
			}
		}
		return peterPaul;
	}
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		if (tempPeter >= 5000 && tempPaul >= 10000) {
			peterPaul.put("Peter", tempPeter - (tempPeter / 4));
			peterPaul.put("Paul", tempPaul - (tempPaul / 4));
			tempPeter = tempPeter / 4;
			tempPaul = tempPaul / 4;
			peterPaul.put("PeterPaulPartnership", tempPeter + tempPaul);
		}
		return peterPaul;
	}
	...
	public Map<String, Integer> wordCount(String[] words) {
		...
		for (int i = 0; i < words.length; i++) {
			String temp = words[i];
			if (uniqueStrings.containsKey(temp)) {
				int count = uniqueStrings.get(temp);
				uniqueStrings.put(temp, count + 1);
			} else {
				uniqueStrings.put(temp, 1);
			}
		}
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		...
		for (int i = 0; i < ints.length; i++) {
			int temp = ints[i];
			if (uniqueNumbers.containsKey(temp)) {
				int count = uniqueNumbers.get(temp);
				uniqueNumbers.put(temp, count + 1);
			} else {
				uniqueNumbers.put(temp, 1);
			}
		}
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		Map<String, Boolean> multiplicityOfString = new HashMap<String, Boolean>();

		for (int i = 0; i < words.length; i++) {
			String temp = words[i];
			if (multiplicityOfString.containsKey(temp)) {
				multiplicityOfString.put(temp, true);
			} else {
				multiplicityOfString.put(temp, false);
			}
		}

		return multiplicityOfString;
	}
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
		...
		for (Map.Entry<String, Integer> kv : remoteWarehouse.entrySet()) {
			
			String sku = kv.getKey();
			Integer value = kv.getValue();

			if (consolidatedInventory.containsKey(sku)) {
				Integer temp = consolidatedInventory.get(sku);
				consolidatedInventory.put(sku, temp + value);
			} else {
				consolidatedInventory.put(sku, value);
			}
		}
	...
	public Map<String, Integer> last2Revisted(String[] words) {
		...
		Map<String, Integer> substringsCount = new HashMap<String, Integer>();
		
		for(String myString : words) {
			
			int counter = 0;
			String lastTwo = myString.substring(myString.length() - 2, myString.length());

			for(int i = 0; i < myString.length() - 2; i++) {
				if(myString.substring(i, i + 2).contains(lastTwo)) {
					counter++;
				}
				substringsCount.put(myString, counter);
			}		
		}
	...
	public List<String> distinctValues(List<String> stringList) {
		...
		stringList.clear();
		stringList.addAll(distinctWords);	
		return stringList;
	}
}
